...
export function formatJobsEmailText(input: {
  ...
  for (const j of newJobs) {
    ...
    if (j.jobUrl && j.jobUrl !== j.applyUrl) lines.push(`Job: ${j.jobUrl}`);
    lines.push("");
  }
  ...
  const htmlItems = newJobs
    .map((j) => {
      const meta: string[] = [];
      if (j.location) meta.push(escapeHtml(j.location));
      if (j.salary) meta.push(escapeHtml(j.salary));
      if (j.tags?.length) meta.push(escapeHtml(j.tags.join(", ")));
      return `
...
function escapeAttr(s: string): string {
  // same escaping is fine for href here (not perfect URL validation, but prevents html breakage)
  return escapeHtml(s);
}
